...
// TODO: Was cleaning up type here, will make singleplayer route
...
const PlayerComp: FC<PlayerProps> = ({ player, mode, self, className }) => {
  //  resize when on phone
  return (
    <div
      className={`flex-col items-center justify-center ${
        player.status == "lose" && mode == "multiple" ? "opacity-50" : ""
      }`}
    >
      ...
      <div
        className={`${className} w-[9rem] h-[4rem] sm:w-[10rem] sm:h-[5rem] md:w-[11rem] md:h-[6rem] lg:w-[12rem] p-2 flex flex-row border-2 border-white rounded-lg justify-center items-center`}
      >
        <motion.div
          className="ml-1"
          initial="hidden"
        ...
        >
          <Man
            className="w-[2.5rem] sm:w-[3.5rem]"
            small
            lives={player.lives}
          />
        </motion.div>
          ...
          >
            ...
            {player.is_host && (
              <Icon
                icon={faCircleH}
                size="lg"
                className=" text-white ml-1 w-3 h-3 sm:w-5 sm:h-5"
              />
            )}
          </motion.div>
          <motion.p
